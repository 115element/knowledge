...
@Slf4j
@Service
public class EsApiStudyService {
    ...
    public void createIndexApi() throws IOException {
        ...
        //可选参数; 超时，等待所有节点被确认,使用TimeValue形式
        request.timeout(TimeValue.timeValueMinutes(2));
        //超时，等待所有节点被确认(使用字符串方式)
        //request.timeout("2m");


        //连接master节点的超时时间(),使用TimeValue形式
        request.masterNodeTimeout(TimeValue.timeValueMillis(1));
        //连接master节点的超时时间(),使用字符串形式
        ...
        //异步执行
        //异步执行创建索引请求，需要将CreateIndexRequest实例和ActionListener实例传递给异步方法；
        //CreateIndexResponse的典型监视器如下所示；
        ActionListener<CreateIndexResponse> listener = new ActionListener<CreateIndexResponse>() {
            @Override
            public void onResponse(CreateIndexResponse e) {
                //如果执行成功，则调用该方法；
            }
        ...
        //返回的CreateIndexResponse允许检测有关执行的操作的信息，如下所示
        //1.指示是否所有节点都已创建请求
        boolean acknowledged = createIndexResponse.isAcknowledged();
        //2.指示是否在超时之前为索引中的每个分片启动了必需的分片副本数
    ...
    public void deleteIndexApi() throws IOException {
        ...
        //指定要删除的索引
        DeleteIndexRequest request = new DeleteIndexRequest("twitter——two");
        //可选参数;等待索引节点确认索引删除超时时间的两种设置方式
        request.timeout(TimeValue.timeValueMinutes(2));
        //request.timeout("2m");

        //设置连接master节点的超时时间的两种方式
    ...
    /**
     * 如果索引已经关闭，再往此索引增加document，
     * 可以看到，返回了一个索引已被关闭的异常，文档无法被正常创建。
     * @throws IOException
     */
    public void openIndexApi() throws IOException {
        //打开索引
        OpenIndexRequest request = new OpenIndexRequest("twitter");

        //可选参数
        //设置超时，等待所有节点确认索引已打开（使用TimeValue形式）
        request.timeout(TimeValue.timeValueMinutes(2));
        //设置超时，等待所有节点确认索引已打开（使用字符串形式）
        //request.timeout("2m");

        //连接master节点的超时时间(使用TimeValue方式)
        request.masterNodeTimeout(TimeValue.timeValueMinutes(2));
        //连接master节点的超时时间(使用字符串方式)
        //request.masterNodeTimeout("2m");
     ...
     * @throws IOException
     */
    public void closeIndexApi() throws IOException {
        //关闭索引
        ...
        //返回的closeIndexResponse允许检索有关执行的操作的信息，如下所示：
        //指示是否所有节点都已确认请求
    ...
    public void singleDocumentApis() throws IOException {
        ...
        //方式3：文档源以XContentBuilder对象的形式提供，Es内部会帮助我们生成JSON内容
        ...
        indexRequest1.opType("create");//字符串方式，可以是create或update(默认)
    ...
    public void getApi() throws IOException {
        ...
        //配置指定store_fields的检索(要求字段在映射中单独存储)
        getRequest.storedFields("message");
        GetResponse getResponse = client.get(getRequest, RequestOptions.DEFAULT);
        //检索message存储字段(要求将字段分开存储在映射中)
