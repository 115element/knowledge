...
public class NeoHttpClient {
	...
	private NeoCookieManager cookieManager;

	public NeoHttpClient() {
		this(false);
	}

	public NeoHttpClient(boolean useCookie) {
		...
		if (useCookie) {
			cookieManager = new NeoCookieManager();
		}
	}
	...
	public Map<String, String> get(String url, Map<String, String> headerMap) throws Exception {
		...
		try (Response response = client.newCall(request).execute()) {
			...
			if (cookieManager != null) {
				cookieManager.storeCookies(url, rspHeaders);
			}
	...
	private Headers buildHeaders(Map<String, String> headerMap, String url) {
		if (cookieManager != null) {
			if (headerMap == null || !headerMap.containsKey("Cookie")) {
