...
public class DailyTest {
    ...
    @Test
    public void showFileTest() throws IOException {
        //在 Java 7 中可以使用 NIO.2 的 API 来做同样的事情(遍历所有文件)，代码如下所示：
        Path initPath = Paths.get("H:\\");
    ...
    @Test
    public void slimit() {
        List<Integer> collect = Stream.of(1, 2, 3, 4, 5, 6).limit(3).peek(System.out::println).collect(Collectors.toList());
        System.out.println(collect);
    }

    @Test
    public void re() {
        boolean date_between = " date_between(2020,2021)".startsWith("date_between");
    ...
    public void tes() {
        int i = 982;
        System.out.println(981 / 100);
    }

    @Test
    public void test1() {
        System.out.println("100000100101101110011100001000010100011100011111000000000001".length());
    }
    ...
    public void bg() {
        ...
        //System.out.println(one - zero); 直接提示编译错误
        BigDecimal subtract = one.subtract(zero); //正确做法
        System.out.println(subtract); //结果： 1

        //BigDecimal a = one / 10; //直接提示编译错误。
        BigDecimal divide = one.divide(BigDecimal.valueOf(10)); //正确做法
        System.out.println(divide);//结果： 0.1
    }
