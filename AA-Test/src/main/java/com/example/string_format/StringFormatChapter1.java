...
//    转换符 ，用于指定格式化的样式，和限制对应入参的数据类型。
    ...
    public void t() {
        ...
        val format = String.format("%2$s %1$s", "111", "222");
...
//    d:结果被格式化为十进制整数
    ...
    public void t1() {
        //格式化数字为八进制
        val format = String.format("%o", 11);
        System.out.println(format);

        //格式化数字为十六进制
        val format1 = String.format("%x", 11);
...
//            至于其它的flag可以查阅JDK文档
    ...
    @Test
    public void t2() {
        val format = String.format("%1$,d", 123025888);
        System.out.println(format);
    }
    ...
    public void t3() {
        ...
        val format1 = String.format("%02d", 1);
        System.out.println(format1);// 01

        val format = String.format("%02d", 111);
...
//    对于字符、整数和日期/时间参数类型转换，以及百分比和行分隔符转换，精度是不适用的；如果提供精度，则会抛出异常。
    ...
    public void t4() {
        //保留两位小数的意思
        val format = String.format("%.2f", 12.12555);
